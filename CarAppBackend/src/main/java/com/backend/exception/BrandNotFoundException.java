package com.backend.exception;

import javax.interceptor.Interceptors;
import javax.ws.rs.core.Response.Status;

import com.backend.entity.Brand;
import com.backend.interceptor.LogInterceptor;

/**
 * Exception thrown when a {@link Brand} is looked for in the system
 * but it hasn't been found
 * @author gmiralle
 */
@Interceptors(LogInterceptor.class)
public class BrandNotFoundException extends CarApiBaseException {

	private static final long serialVersionUID = 3715877372548587658L;
	private static final String DEFAULT_MESSAGE = "The requested brand couldn't be found. ";

	/**
	 * Create an exception with the a default message<br>
	 * <b>"The requested brand couldn't be found. "</b>
	 */
	public BrandNotFoundException() {
		super(DEFAULT_MESSAGE, Status.NOT_FOUND);
	}
	
	/**
	 * Create an exception with the default message and the Brand's ID
	 * that provoked it
	 * @param id Brand's ID that provoked the exception
	 */
	public BrandNotFoundException(Integer id) {
		super(DEFAULT_MESSAGE + id, Status.NOT_FOUND);
	}
}
